...
def render_command_center(daily_command: Dict[str, Any]) -> str:
    ...
    <style>
        ...
        .dc-hero {background:linear-gradient(128deg,#07111f,#0b2443 52%,#146c65); border:1px solid rgba(96,165,250,.45); border-radius:24px; padding:20px; box-shadow:0 18px 50px rgba(0,0,0,.36);}
        .dc-kicker {font-size:.73rem; letter-spacing:.2em; color:#86efac; font-weight:900; text-transform:uppercase;}
        .dc-title {font-size:2.25rem; color:#ffffff; font-weight:900; line-height:1.02; margin-top:8px;}
        ...
        .dc-mission-title {font-size:1.95rem;color:#fff;font-weight:900;margin-top:8px;}
        ...
        .dc-primary-shell {background:linear-gradient(132deg,#0b1b2f,#143965); border:1px solid rgba(96,165,250,.45); border-radius:18px; padding:12px; margin-top:12px;}
        .dc-secondary-note {font-size:.78rem;color:#9cc7ff;margin-top:8px;}
    ...
    st.markdown(
        ...
        <div class="dc-mission">
            ...
            <div class="dc-grid" style="margin-top:10px;">
                ...
                <div class="dc-chip"><div class="dc-chip-label">Recovery</div><div class="dc-chip-value">{_text(command.get('readiness_label'), 'Unknown')}</div></div>
                ...
                <div class="dc-chip"><div class="dc-chip-label">Latest PR</div><div class="dc-chip-value">{_metric(snapshot.get('latest_pr'))}</div></div>
            </div>
            <div style="margin-top:10px;color:#cfe4ff;">Today's focus: {_text(command.get('recommended_focus'), 'Not available')} • Confidence {confidence_text}</div>
            <div style="margin-top:6px;color:#cfe4ff;">Reason: {_text(command.get('main_reason'), 'Recommendation based on readiness and recent workload.')}</div>
    ...
    action = ''
    st.markdown('<div class="dc-primary-shell">', unsafe_allow_html=True)
    if st.button("START TODAY'S WORKOUT", width='stretch', key='dc_primary_start', type='primary'):
        action = 'start_workout'
    st.markdown('<div class="dc-secondary-note">Secondary actions</div>', unsafe_allow_html=True)
    a2, a3, a4 = st.columns(3)
    if a2.button('Preview Workout', width='stretch', key='dc_secondary_preview'):
        action = 'preview_workout'
    if a3.button('Adjust Plan', width='stretch', key='dc_secondary_adjust'):
        action = 'adjust_plan'
    if a4.button('Recovery Instead', width='stretch', key='dc_secondary_recovery'):
